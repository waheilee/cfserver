...
class YdpayNotify extends Controller
{
    ...
    public function notify()
    {
        try {
            ...
            save_log('ydpay', 'paynotify:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            ...
            $realmoney     = $params['pay_amount'];
            $transactionId = $params['order_no'];
            $status        = $params['order_status']==4 ? 1 : 0;
            ...
            $checksign = strtolower(md5($detail['appid'].$orderid.$detail['secret']));
            $text = 'success';
            ...
            save_log('ydpay', 'Exception:' . $ex->getMessage() . $ex->getLine() . $ex->getTraceAsString());
            exit('OK');
        }
    }
    ...
    public function outnotify(){
        $params = $_POST ?: json_decode(file_get_contents('php://input'),1);
        try {
            save_log('ydpay','代付回调:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            $json = json_encode($params);
            ...
            $mysign = $this->rsaPublicDecrypt($sign,$channelcfg['public_key']);
            if ($mysign) { //验签 Verify signature
                ...
                $gameoc->PaynotifyLog()->Insert($data);
                save_log('ydpay','更新订单状态并写日志:' . json_encode($data));
    ...
    public function rsaPublicDecrypt($data,$key)
    {

        $decrypted = '';
        $pi_key = "-----BEGIN PUBLIC KEY-----\n";
        ...
        foreach ($plainData as $chunk) {
            $str = '';
            //私钥解密
            $decryptionOk = openssl_public_decrypt($chunk, $str, $pi_key);
