...
class WinpayNotify extends Controller
{

    public function notify()
    {
        try {
            $params = $_POST ?: json_decode(file_get_contents('php://input'),1);
            save_log('winpay', 'paynotify:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            ...
            $checksign = md5($detail['appid'].$orderid.$realmoney.$detail['secret']);
            $text = 'success';
            ...
            save_log('winpay', 'Exception:' . $ex->getMessage() . $ex->getLine() . $ex->getTraceAsString());
            exit('OK');
        }
    }


    //{"channelId":"123","channleOid":"202211211404381890055","channelOid":"202211211404381890055","amount":"100.00","oid":"SATP6576414824dd","status":3,"sign":"59eeab44be0e9adc4667ef8ae458dcff"}
    public function outnotify(){
        $params = $_POST ?: json_decode(file_get_contents('php://input'),1);
        try {
            save_log('winpay','代付回调:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            $json = json_encode($params);
            ...
            $mysign = md5($channelcfg['appid'].$orderid.$realmoney.$channelcfg['secret']);
            if ($sign == $mysign) { //验签 Verify signature
                ...
                if (trim($status) == '1') {
                    ...
                    $text = "OK";
                    save_log('winpay','通知服务器状态:' . json_encode($res));
                } elseif (trim($status) == '2' || trim($status) == '3') {
                    //金币退回
                ...
                $gameoc->PaynotifyLog()->Insert($data);
                save_log('winpay','更新订单状态并写日志:' . json_encode($data));
