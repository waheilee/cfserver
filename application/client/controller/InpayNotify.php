...
class InpayNotify extends Controller
{
    ...
    public function notify()
    {
        try {
            $params =$_POST;
            save_log('inpays', 'paynotify:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            $sign = $params['sign'];
            ...
            $realmoney = $params['total_fee'];
            $transactionId ='RE_'.$orderid;
            ...
            if ($sign == $checksign) { //验签 Verify signature
                ...
                if (trim($status) == 'payin_success') {
                    ...
                    save_log('inpays', '玩家id:' . $order['AccountID'] . ',订单id:' . $transactionId . ',金币发送状态' . $code);
                    if (intval($code) === 0) {
                        $data = [
            ...
            save_log('inpays', 'Exception:' . $ex->getMessage() . $ex->getLine() . $ex->getTraceAsString());
            exit('success');
        }
    }
    ...
    public function outnotify(){
        ...
        try {
            save_log('inpays','payoutnotify:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            $json = json_encode($params);
            ...
            $checksign = $this->genSign($params, $channelcfg['secret']);
            if ($sign == $checksign) { //验签 Verify signature
                $order_status =0;
                $log_txt ='';
                ...
                if (trim($status) === 'payout_success') {
                    ...
                    $order_status =5;
                    $log_txt='第三方处理中'.$params['fail_info'];
