...
class SsspayNotify extends Controller
{
    ...
    public function notify()
    {
        try {
            ...
            $transactionId = $params['platformOrderid'];
            $sign = $params['sign'];
            unset($params['sign']);
            unset($params['attach']);
            ...
            if ($checksign==$sign) { //验签 Verify signature
                ...
                if ($status == 0) {
                    ...
                    save_log('ssspay', '玩家id:' . $order['AccountID'] . ',订单id:' . $transactionId . ',金币发送状态' . $code);
                    if (intval($code) === 0) {
                        $data = [
