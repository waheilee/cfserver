...
class SerpayNotify extends Controller
{
    ...
    public function notify()
    {
        try {
            $params =$_POST;
            save_log('serpay', 'paynotify:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            $sign = $params['sign'];
            ...
            $orderid =str_replace('NEICHONG_','',$params['custOrderNo']);
            $realmoney =bcdiv($params['ordAmt'],100,0);
            $transactionId = $params['prdOrdNo'];
            ...
            if ($sign == $checksign) { //验签 Verify signature
                ...
                if (trim($status) == '01') {
                    ...
                    save_log('serpay', '玩家id:' . $order['AccountID'] . ',订单id:' . $transactionId . ',金币发送状态' . $code);
                    if (intval($code) === 0) {
                        $data = [
                    ...
                    $user = new UserDB();
                    $ret = $user->updateTable('T_UserChannelPayOrder', $data, ['OrderId' => $orderid]);
                    $log_txt = '第三方订单处理失败' . $params['respCode'];
            ...
            save_log('serpay', 'Exception:' . $ex->getMessage() . $ex->getLine() . $ex->getTraceAsString());
            exit('SC000000');
        }
    }
    ...
    public function outnotify(){
        ...
        try {
            save_log('serpay','sepronotify:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            $json = json_encode($params);
            ...
            $orderid = $params['custOrderNo'];
            $amount = bcdiv($params['payAmt'],100,0);
            $status = $params['ordStatus'];
