...
class HwepayNotify extends Controller
{
    ...
    public function notify()
    {
        try {
            $params =$_POST;
            save_log('hwepay', '代收通知:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            $sign = $params['sign'];
            ...
            $checksign = md5($transactionId.$merchant_no.$orderid.$realmoney.$pay_type.$code.$detail['secret']);
            $text = 'success';
            if ($sign == $checksign) { 
                $log_txt = '';
                if ($code ==1) {
            ...
            save_log('hwepay', 'Exception:' . $ex->getMessage() . $ex->getLine() . $ex->getTraceAsString());
            exit('success');
        }
    }
    ...
    public function outnotify(){
        ...
        try {
            save_log('hwepay','代付回调:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            $json = json_encode($params);
            ...
            $mysign = md5($transactionId.$merchant_no.$orderid.$realmoney.$pay_type.$code.$channelcfg['secret']);
            if ($sign == $mysign) { //验签 Verify signature
                ...
                if (trim($status) == '1') {
                    ...
                    $text = "success";
                    save_log('hwepay','通知服务器状态:' . json_encode($res));
                ...
                $gameoc->PaynotifyLog()->Insert($data);
                save_log('hwepay','更新订单状态并写日志:' . json_encode($data));
