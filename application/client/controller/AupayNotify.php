...
class AupayNotify extends Controller
{
    ...
    public function notify()
    {
        try {
            $params =$_POST;
            save_log('aupay', 'paynotify:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            $sign = $params['sign'];
            ...
            $realmoney = $params['true_amount']/100;
            ...
            if ($sign == $checksign) { //验签 Verify signature
                ...
                if (trim($status) == 'success') {
                    ...
                    save_log('aupay', '玩家id:' . $order['AccountID'] . ',订单id:' . $transactionId . ',金币发送状态' . $code);
                    if (intval($code) === 0) {
                        $data = [
            ...
            save_log('aupay', 'Exception:' . $ex->getMessage() . $ex->getLine() . $ex->getTraceAsString());
            exit('OK');
        }
    }
    ...
    public function outnotify(){
        ...
        try {
            save_log('aupay','payoutnotify:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            $json = json_encode($params);
            ...
            $orderid = $params['userordernumber'];
            ...
            ksort($params);
            $stra =http_build_query($params);
            $stra = urldecode($stra);
            $signValue = md5($stra."&key=".$channelcfg['secret']);
            if ($sign == $signValue) { //验签 Verify signature
                $order_status =0;
                $log_txt ='';
                $transactionId =$params['remarks'];
