...
class SeproNotify extends Controller
{
    ...
    public function notify()
    {
        try {
            $params =$_POST;
            save_log('sepropay', 'paynotify:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            ...
            save_log('sepropay', 'Exception:' . $ex->getMessage() . $ex->getLine() . $ex->getTraceAsString());
            exit('success');
        }
    }
    ...
    public function outnotify(){
        ...
        try {
            save_log('sepropay','sepronotify:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            $json = json_encode($params);
