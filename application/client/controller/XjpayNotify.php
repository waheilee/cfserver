...
class xjpayNotify extends Controller
{
    ...
    public function notify()
    {
        try {
            ...
            save_log('xjpay', 'paynotify:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            ...
            $status        = $params['orderStatus'] == 'SUCCESS' ? 1 : 0;
            ...
            $checksign     = hash("sha512", $json.$detail['secret']);
            $text = 'success';
            if ($sign == $checksign) { //验签 Verify signature
                ...
                if (trim($status) == 1) {
                    ...
                    $user = new UserDB();
                    $ret = $user->updateTable('T_UserChannelPayOrder', $data, ['OrderId' => $orderid]);
                    $log_txt = '第三方订单处理失败' . $params['orderStatus'];
            ...
            save_log('xjpay', 'Exception:' . $ex->getMessage() . $ex->getLine() . $ex->getTraceAsString());
            exit('OK');
        }
    }
    ...
    public function outnotify(){
        $params = $_POST ?: json_decode(file_get_contents('php://input'),1);
        try {
            save_log('xjpay','代付回调:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            $json = json_encode($params);
            ...
            if ($sign == $checksign) { //验签 Verify signature
                ...
                if (trim($status) == '1') {
                    ...
                    //金币退回
                    $log_txt='第三方处理失败'.$params['failMessage'];
                    $order_status =5;
                    $text = "Reject"; //处理失败 Processing failed
                }
                else{
                    $order_status =3;
                    $log_txt='第三方处理中'.$params['failMessage'];
                ...
                $gameoc->PaynotifyLog()->Insert($data);
                save_log('xjpay','更新订单状态并写日志:' . json_encode($data));
