...
class AbcpayNotify extends Controller
{
        ...
        try {
            ...
            if ($checksign==$sign) { //验签 Verify signature
                ...
                if ($status == 200) {
                    ...
                    save_log('abcpay', '玩家id:' . $order['AccountID'] . ',订单id:' . $transactionId . ',金币发送状态' . $code);
                    if (intval($code) === 0) {
                        $data = [
            ...
            save_log('abcpay', 'Exception:' . $ex->getMessage() . $ex->getLine() . $ex->getTraceAsString());
            exit('success');
        }
    }
