...
class HolidiapayNotify extends Controller
{
    ...
    public function notify()
    {
        try {
            ...
            save_log('holidiapay', 'paynotify:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            ...
            if ($sign == $checksign) { //验签 Verify signature
                ...
                if (trim($status) == 1) {
                    ...
                    save_log('holidiapay', '玩家id:' . $order['AccountID'] . ',订单id:' . $transactionId . ',金币发送状态' . $code);
                    if (intval($code) === 0) {
                        $data = [
            ...
            save_log('holidiapay', 'Exception:' . $ex->getMessage() . $ex->getLine() . $ex->getTraceAsString());
            exit('SUCCESS');
        }
    }
    ...
    public function outnotify(){
        $params = $_POST ?: json_decode(file_get_contents('php://input'),1);
        try {
            save_log('holidiapay','代付回调:' . json_encode($params));
            if (empty($params)) {
                exit('Empty Request');
            }
            $json = json_encode($params);
            ...
            if ($sign == $checksign) { //验签 Verify signature
                ...
                if (trim($status) == '1') {
                    ...
                    $text = "OK";
                    save_log('holidiapay','通知服务器状态:' . json_encode($res));
                ...
                $gameoc->PaynotifyLog()->Insert($data);
                save_log('holidiapay','更新订单状态并写日志:' . json_encode($data));
