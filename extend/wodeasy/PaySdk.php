...
class PaySdk
{
    ...
    public function pay($param, $config = [])
    {
        ...
        $data = [
            'type'=>'upi',
            'id' =>$this->merchant,
            'order' =>trim($param['orderid']),
            'amount' =>sprintf('%.2f',$param['amount']) ,
            'notes' =>'{"email":"dfdfd@gmail.com","name":"ggg","mobile":"9874561230"}',
        ...
        $result =$this->curl_post_content($this->api_url .'/shopin',http_build_query($data), $header);
        save_log('wodeasy','提交参数:'.json_encode($data).',接口返回信息：'.$result);
        $res = json_decode($result, true);
        if (!isset($res['code'])) {
            $res['message'] ='Http Request Invalid';
            //exit('Http Request Invalid');
        }
        //{"code":0,"platformOrderid":"165794044655867121","backUrl":"http://www.wodeasy.org/starpay/pay/order/webNowOpen/165794044655867121","payLink":""}
        $returl='';
        if(is_array($res)){
            if($res['code']=='success')
                $returl= $res['url'];
