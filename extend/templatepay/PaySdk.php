...
class PaySdk
{
    ...
    public function notify($params,$header=[],$channel,$logname)
    {
        try {
            ...
            $data['orderid']       = $params['result']['out_trade_no']??'';
            $data['realmoney']     = $params['result']['amount']??'';
            $data['transactionId'] = $params['result']['trans_no']??'';
            $data['status']        = $params['result']['status']??'' == '1' ? 1 : 0;
            //sign认证
            $checksign             = $this->createSign($params['result'],$channel['secret']);
    ...
    public function outnotify($params,$header=[],$channel,$logname){
        try {
            ...
            $data['orderid']       = $params['result']['out_trade_no']??'';
            $data['transactionId'] = $params['result']['trans_no']??'';
            $data['code']          = $params['result']['payout_status']??'';
            switch ($data['code']) {
                case '1':
                    //成功
                    $data['status'] = 1;
