...
//商户私钥
    ...
    public function pay($param, $config = [])
    {
        ...
        $data = [
            'merId' => $this->appid,               //商户号
            'orderId' =>$param['orderid'],            //订单号，值允许英文数字
            'orderAmt' =>sprintf('%.2f',$param['amount']),              //订单金额,单位元保留两位小数
            'channel' => 'IND0',            //支付通道编码
            'desc' => $param['roleid'],           //简单描述，只允许英文数字 最大64
            'ip' => $clientIP,          //用户的ip地址必传，风控需要
            'notifyUrl' =>$config['notify_url'],   //异步返回地址
            'returnUrl' => $config['redirect_url'],     //同步返回地址
        ...
        $result =$this->curl_post_content($this->api_url .'/pay', http_build_query($data), $header);
