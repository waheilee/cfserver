...
class PaySdk
{
    ...
    {
        if (isset($config['merchantid']) && !empty($config['mchid'])) {
        ...
        save_log('icbcpay','提交参数:'.json_encode($data).',接口返回信息：'.$result);
        ...
        //$returl =str_replace('https','orange',$returl);
        return $returl;
    }
    ...
    private  function  getToken($param,$config = []){
        ...
        save_log('icbcpay','提交参数:'.json_encode($postadata).',接口返回信息：'.$result);
        $res = json_decode($result, true);
        $token ='';
        if (!isset($res['code'])) {
            return '';
