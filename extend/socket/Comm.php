...
define('REQ_SAFETY', 13);	//安全服务器

//config('socket_ip','192.168.0.155');

//帐号服务器地址
define('AS_SERVER_IP', config('socket_account'));
define('AS_SERVERPORT', 18900);

//数据中心地址
define('DC_SERVER_IP', config('socket_ip'));
define('DC_SERVERPORT', 18600);

//订单服务器地址
define('OS_SERVER_IP', config('socket_ip'));
...
class Comm
{
    function getSocketInstance($str = 'DC') {
        ...
        switch ($str){
            case 'DC' : return new MySocket(DC_SERVER_IP, DC_SERVERPORT);
            case 'AS' : return new MySocket(AS_SERVER_IP, AS_SERVERPORT);
            case 'OS' : return new MySocket(OS_SERVER_IP, OS_SERVERPORT);
            default :
                return  null;
        }
    }
    ...
    /**
     * @param int $type 消息号 CMD_MD_TEST_REQ
     * @param int $len  消息长度
     * @param int $ret  返回标志、路径等
     * @param int $src   REQ_OM  从哪里来
     * @param int $dest  REQ_DC 到哪里去
     * @return false|string
     */
